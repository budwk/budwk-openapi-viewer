    ...
    if (Object.keys(requestBody).length === 0 || Object.keys(requestBody[reqBodyMimeType]).length === 0) {
        ...
        if (requestBody[reqBodyMimeType].examples[0]) {
            if (reqBodyMimeType.toLowerCase().includes("json")) {
                try {
                    reqBodyContent = JSON.parse(requestBody[reqBodyMimeType].examples[0].exampleValue);
                }
                catch{
                    reqBodyContent = "";
                }
                ...
                reqBodyContent = requestBody[reqBodyMimeType].examples[0].exampleValue;
            }
        }
        // 转换表单参数
        if ('application/x-www-form-urlencoded' === reqBodyMimeType) {
            if (typeof reqBodyContent === 'string') {
                reqBodyContent = JSON.parse(reqBodyContent);
            }
