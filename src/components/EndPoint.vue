    ...
    <div
      ...
      :class="
        'sw-endpoint-wrapper ' +
          fullPath.method +
          ' ' +
          (fullPath.expanded ? 'sw-expanded' : 'sw-collapsed')
      "
    >
      <!-- EndPoint Head -->
      <div
      ...
      >
        ...
        <div
          :class="'sw-path ' + (fullPath.depricated ? ' sw-depricated' : '')"
        >
          {{ fullPath.path }}
        </div>
        <span
          style="font-size:12px; text-transform:uppercase; font-weight:bold; color:orangered; margin:2px 0 0 5px;"
          v-if="fullPath.depricated"
        >
          Depricated
        </span>
      ...
      <!-- EndPoint Body -->
      <div
        v-if="fullPath.expandedAtLeastOnce"
        v-show="fullPath.expanded"
        :class="'sw-endpoint-body ' + fullPath.method"
      >
        <div
          class="sw-end-point-summary"
          v-if="fullPath.summary || fullPath.description"
        >
...
.sw-endpoint-body {
  ...
  &.delete {
    border-top: 1px solid $sw-red;
  }
  &.patch {
    border-top: 1px solid $sw-icon-warn-minor;
  }
  &.put {
    border-top: 1px solid $sw-orange;
  }
  &.post {
    border-top: 1px solid $sw-info;
  }
  &.get {
    border-top: 1px solid $sw-green;
  }
}
