<template>
  ...
  >
    ...
    >
      <div class="sw-side-panel-container">
        <div class="sw-row">
          ...
          >
            <el-option
              v-for="item in parsedSpec.servers"
              :key="item.url"
              :label="item.url"
              :value="item.url"
            ></el-option>
    ...
    <!-- Load JSON Panel -->
    <vue-sideout-panel
      v-model="showLoadJsonPanel"
      @close="showLoadJsonPanel = false"
      :widths="['500px']"
      style="z-index:9000"
    >
      <div class="sw-side-panel-container">
        <div class="sw-bold-small-text">粘贴JSON字符串</div>
        <textarea
          class="sw-mono-font"
          v-model="jsonSpecText"
          style="flex:1"
          @keyup.enter="loadJson()"
        />
        <button
          class="sw-btn sw-primary"
      ...
      <div class="sw-row">
        ...
        <div style="margin: 0px 8px;">
          <input
            ...
            class="sw-spec-url sw-dark sw-medium"
            v-model="specUrl"
            @keyup.enter="loadSpec(false)"
          />
          <button
            class="sw-btn sw-primary"
            style="border-radius: 0 2px 2px 0; padding-left:5px; padding-right:5px;"
            @click="loadSpec(false)"
          >
            OPEN
          </button>
        </div>
        ...
        <div
          style="display:flex; flex-direction:column; margin-right:8px; align-items:flex-end;"
        >
          <input
            style="width:100px;"
            type="text"
            placeholder="Search"
            class="sw-medium sw-dark"
            v-model="searchVal"
            @keyup="onSearchKeyUp"
          />
        </div>
        <div
          style="display:flex; flex-direction:column; margin-right:8px; align-items:flex-start;"
        >
          <!--
            <el-switch 
              v-model="expandAll" 
          ...
          <i
            style="color:#aaa; font-size:32px; cursor:pointer"
    ...
    <div ref="pageContainer" class="sw-page-container">
      ...
      <div class="sw-doc-info" v-if="parsedSpec.info">
        <div class="sw-doc-title">
          ...
          <span
            v-if="parsedSpec.info.version"
            style="font-size:14px;font-weight:bold"
            >{{ parsedSpec.info.version }}</span
          >
        </div>
        ...
        <div
          v-if="parsedSpec.info.license && parsedSpec.info.license.name"
          style="font-size:12px;margin:8px 0 0 0;"
        >
          {{ parsedSpec.info.license.name }}
          <a
            v-if="parsedSpec.info.license.url"
            :href="parsedSpec.info.license.url"
          >
            {{ parsedSpec.info.license.url }}</a
          >
        </div>
      </div>
      ...
      <security-schemes
        v-if="parsedSpec.securitySchemes"
  ...
  methods: {
    ...
    afterSpecParsedAndValidated(spec, isReloadingSpec = false) {
      ...
      if ((spec.servers && spec.servers.length == 0) || !spec.servers) {
        serverUrl = me.specUrl.substring(
          0,
          me.specUrl.indexOf("/", me.specUrl.indexOf("//") + 2)
        );
        if (spec.basePath) {
      ...
      store.commit("selectedApiServer", selectApiServerUrl);
